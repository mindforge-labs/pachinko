...
type GrainientProps = {
  timeSpeed?: number; colorBalance?: number; warpStrength?: number; warpFrequency?: number;
  warpSpeed?: number; warpAmplitude?: number; blendAngle?: number; blendSoftness?: number;
  rotationAmount?: number; noiseScale?: number; grainAmount?: number; grainScale?: number;
  grainAnimated?: boolean; contrast?: number; gamma?: number; saturation?: number;
  centerX?: number; centerY?: number; zoom?: number; color1?: string; color2?: string;
  color3?: string; className?: string;
};
...
export default function Grainient({
  ...
  color2 = '#5227FF', color3 = '#B497CF', className = '',
}: GrainientProps) {
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uTimeSpeed: { value: 0.25 }, uColorBalance: { value: 0 }, uWarpStrength: { value: 1 },
        uWarpFrequency: { value: 5 }, uWarpSpeed: { value: 2 }, uWarpAmplitude: { value: 50 },
        uBlendAngle: { value: 0 }, uBlendSoftness: { value: 0.05 }, uRotationAmount: { value: 500 },
        uNoiseScale: { value: 2 }, uGrainAmount: { value: 0.1 }, uGrainScale: { value: 2 },
        uGrainAnimated: { value: 0 }, uContrast: { value: 1.5 }, uGamma: { value: 1 },
        uSaturation: { value: 1 }, uCenterOffset: { value: new Float32Array([0, 0]) },
        uZoom: { value: 0.9 }, uColor1: { value: new Float32Array([1, 1, 1]) },
    ...
    const start = () => { if (inView && pageVisible && frame === 0) frame = requestAnimationFrame(loop); };
    const stop = () => { if (frame) cancelAnimationFrame(frame); frame = 0; };
    const intersectionObserver = new IntersectionObserver(([entry]) => { inView = entry.isIntersecting; inView ? start() : stop(); });
  ...
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    const uniforms = contexts.get(container)?.program.uniforms;
    if (!uniforms) return;
    Object.entries({ uTimeSpeed: timeSpeed, uColorBalance: colorBalance, uWarpStrength: warpStrength, uWarpFrequency: warpFrequency, uWarpSpeed: warpSpeed, uWarpAmplitude: warpAmplitude, uBlendAngle: blendAngle, uBlendSoftness: blendSoftness, uRotationAmount: rotationAmount, uNoiseScale: noiseScale, uGrainAmount: grainAmount, uGrainScale: grainScale, uGrainAnimated: grainAnimated ? 1 : 0, uContrast: contrast, uGamma: gamma, uSaturation: saturation, uZoom: zoom }).forEach(([key, value]) => { uniforms[key].value = value; });
